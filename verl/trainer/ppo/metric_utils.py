...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    structure_adv = batch.batch.get("advantages_structure_select")
    if structure_adv is not None:
        valid_structure_adv = torch.masked_select(structure_adv, response_mask)
        if valid_structure_adv.numel() > 0:
            metrics.update(
                {
                    "critic/advantages/structure_select/abs_mean": torch.mean(torch.abs(valid_structure_adv)).detach().item(),
                    "critic/advantages/structure_select/max": torch.max(valid_structure_adv).detach().item(),
                    "critic/advantages/structure_select/min": torch.min(valid_structure_adv).detach().item(),
                }
            )
    correctness_adv = batch.batch.get("advantages_correctness")
    if correctness_adv is not None:
        valid_correctness_adv = torch.masked_select(correctness_adv, response_mask)
        if valid_correctness_adv.numel() > 0:
            metrics.update(
                {
                    "critic/advantages/correctness/abs_mean": torch.mean(torch.abs(valid_correctness_adv)).detach().item(),
                    "critic/advantages/correctness/max": torch.max(valid_correctness_adv).detach().item(),
                    "critic/advantages/correctness/min": torch.min(valid_correctness_adv).detach().item(),
                }
            )
    task_types = batch.non_tensor_batch.get("task_type", None)
    task_types_arr = None
    if task_types is not None:
        try:
            ...
            task_types_arr = None
    if task_types_arr is not None and len(task_types_arr) == advantages.shape[0]:
        for task_type in ("categorical", "numerical"):
            sample_mask = torch.as_tensor(task_types_arr == task_type, device=advantages.device)
            if not torch.any(sample_mask):
                continue
            type_mask = response_mask & sample_mask[:, None]
            type_adv = torch.masked_select(advantages, type_mask)
            if type_adv.numel() == 0:
                continue
            metrics.update(
                {
                    f"critic/advantages/{task_type}/abs_mean": torch.mean(torch.abs(type_adv)).detach().item(),
                    f"critic/advantages/{task_type}/max": torch.max(type_adv).detach().item(),
                    f"critic/advantages/{task_type}/min": torch.min(type_adv).detach().item(),
                }
            )
    return metrics
